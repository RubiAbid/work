...
import { FaPhoneAlt, FaMapMarkerAlt, FaFacebookF, FaTwitter, FaYoutube, FaWordpress, FaChevronDown, FaSearch, FaUser, FaHeart, FaShoppingBasket } from 'react-icons/fa';
      ...
      <div className="bg-white py-4 shadow-md">
        <div className="container mx-auto flex justify-between items-center px-4">
          ...
          <div className="flex items-center space-x-4">
            <Image src="/poco.png" alt="Poco! logo" width={100} height={50} className="h-10" />
            <nav className="flex space-x-6">
              <NavItem href="#" label="Home" icon={<FaChevronDown />} active />
              <NavItem href="#" label="Menu" />
              <NavItem href="#" label="About" />
              <NavItem href="#" label="Shop" icon={<FaChevronDown />} />
              <NavItem href="#" label="Blog" icon={<FaChevronDown />} />
              <NavItem href="#" label="Page" icon={<FaChevronDown />} />
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-4">
              ...
              <HeaderIcon icon={<FaHeart />} count={0} />
              <HeaderIcon icon={<FaShoppingBasket />} count={0} />
            </div>
          </div>
        </div>
      </div>
...
function NavItem({ href, label, icon, active }: NavItemProps) {
  return (
    <Link href={href} className={cn("flex items-center space-x-1", active ? "text-yellow-500" : "text-black")}> 
      <span>{label}</span>
      {icon && <span>{icon}</span>}
    </Link>
  );
